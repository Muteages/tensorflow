...
absl::Status MetaOptimizer::InitializeCustomGraphOptimizers(
    ...
    std::vector<std::unique_ptr<GraphOptimizer>>* optimizers) const {
  bool freeze_readonly_variables_grappler_initialized =
      pre_initialized_optimizers.find(
          FreezeReadonlyVariablesGrapplerOptimizerName()) !=
  ...
  for (const auto& optimizer_config : cfg_.custom_optimizers()) {
    ...
    if (custom_optimizer) {
      ...
      optimizers->push_back(std::move(custom_optimizer));
      if (optimizer_config.name() ==
          FreezeReadonlyVariablesGrapplerOptimizerName()) {
        freeze_readonly_variables_grappler_initialized = true;
      }
    } else {
  ...
  if (IsFreezeReadonlyVariablesGrapplerEnabled() &&
      !freeze_readonly_variables_grappler_initialized) {
    auto optimizer = CreateFreezeReadonlyVariablesGrapplerOptimizer();
